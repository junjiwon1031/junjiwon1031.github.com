...
- Stem Block 을 Dense Block 전에 둔다.
  ...
  PeleeNet 의 특징이라면 다른 구조들은 단순 convolution 쌓기 구조라면, 여기서는 Resblock 을 사용한다는 것이다.
...
- Transition Layer 의 Compression 을 사용하지 않는다.
  ...
  DenseNet 논문을 읽어보면 Compression만 성능 비교를 한 부분이 없긴 하다. 치사하게 Bottleneck 이랑 묶어 놔서 알 수가 없다.
...
- 38 x 38 feature map... 안써요 어휴
  ...
  Pelee 는 이에 따라 38 x 38 을 빼고 19 x 19 에서 그 역할을 같이하는 것으로 나머지 5개만 prediction 에 사용한다.
...
- Small Convolutional kernel Prediction
  ...
  저자들은 3 x 3 대신 1 x 1 로 대신하여 거의 비슷한 결과를 얻으면서도 21.5% cost 하락을 이끌어 내었다.
...
![이미지넷 결과](/assets/pelee_results_ILSVRC2012.PNG)

![tx2 스피드 결과](/assets/pelee_results_speed_tx2.PNG)
...
## Conclusion
  ...
  다만 아쉬운 점이 있다면 evaluation 이 빈약하다는 것이다.

  DenseNet-41 에서 변형시켜서 만들어진 PeleeNet 의 layer는 *47*개이다. (??)

  Performance를 비교할 때 가장 마지막 단에 **Go deeper(dense layer 3개 추가)** 를 넣었기 때문이다.

  그렇다면 비교해야하는 대상은 DenseNet-41 이 아니고 DenseNet-47 이 되어야하는 것이 아닐까?

  DenseNet-47 과 비교했을 때 실제로 성능이 그만한 향상을 보일 수 있을까? 이 논문을 통해서는 알 수가 없다...
